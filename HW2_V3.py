...
def createTriangles(data, hull, resultPath, filename, visulization = True):
    triangleDict = {}
    ...
    if visulization:
        caption = 'Note: Some of the boundary points are coincide with the vertices of the convex hull'
...
def isConvex(data, boundaryPointsDict, triangleDict, demo = True):
    ...
    for bdrPntIdx in list(boundaryPointsDict.keys()):
        ...
        if not PntonConvexFlag:
            ...
            if demo:
                ...
                plt.title('Demo showing that which point is not on the convex hull')
                plt.legend(loc='lower right')
...
for file in files:
    ...
    resultPath = resultFolder + filename
    ...
    saveDecimatedFile(resultPath, boundaryPointsDict, filename+'Decimated')
    ...
    hull, convexHullpts = createConvexHull_Qhull(data, filename, savefig = False)

    # Create, plots and saves triangles of the convex hull which is obtained from previous step
    triangleDict = createTriangles(data, hull, resultPath, filename, visulization = True)


    convexity = isConvex(data, boundaryPointsDict, triangleDict, demo = True)
