...
python_vertices = np.zeros([len(hull.vertices),3])
vv = 0

for v in hull.vertices:
    python_vertices[vv,:] = data[v]['Coordinate']
    vv+=1
...
# points Coordinates
for i in indexList:
    points.append(Point(data[i]['Coordinate'][0], data[i]['Coordinate'][1], data[i]['Coordinate'][2]))
...
alg_vertices = np.zeros([len(final_vertices), 3])

for v in range(len(final_vertices)):
    alg_vertices[v,0] = list(final_vertices)[v].x
    alg_vertices[v,1] = list(final_vertices)[v].y
    alg_vertices[v,2] = list(final_vertices)[v].z


# def isEqual(alg_vertices, python_vertices):


list1 = []
list2 = []

alg_v_point_numbers = alg_vertices.shape[0]
python_v_point_numbers = python_vertices.shape[0]

# if alg_v_point_numbers != python_v_point_numbers:
#     return False

for i in range(alg_vertices.shape[0]):
    list1.append(tuple(alg_vertices[i,:]))
    list2.append(tuple(python_vertices[i,:]))
